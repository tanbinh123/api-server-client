  ...
  <div>
    <keep-alive>
      ...
      <a-card v-if="$route.name === 'User'" :bordered="false">
        ...
        <div v-if="checkPermission(['sysUser:add'])" class="table-operator">
        ...
        <a-table
          ...
          <template slot="action" slot-scope="record">
            <div class="row-operations">
              <a-tooltip
                v-permission="['sysUser:toggle']"
                :title=" record.state==='0'?'启用':'禁用' ">
                <a-switch size="small" :checked="record.state === '0'" @change="toggleState(record)" />
              </a-tooltip>
              <a-button class="rowBtn" size="small" type="link" v-permission="['sysUser:update']" @click="handleEdit(record)">编辑</a-button>
              <a-button
                v-permission="['sysUser:configRoles']"
                class="rowBtn"
                type="link"
                size="small"
                @click="handleConfigRoles(record.id)">配置角色</a-button>
              <a-popconfirm
                v-permission="['sysUser:resetPwd']"
                placement="top"
                title="确定重置此用户密码吗？"
                trigger="hover"
...
import SearchForm from '@/components/Service/SearchForm'
const modalSetting = modalForm()
