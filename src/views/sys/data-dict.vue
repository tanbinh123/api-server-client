  ...
  <a-card :bordered="false">
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row :gutter="20">
          <a-col v-if="checkPermission(['sysDict:add'])" :md="10" :sm="24">
    ...
    <a-table
      ...
      <template slot="action" slot-scope="record">
        <div class="row-operations">
          <a-tooltip v-permission="['sysDict:toggle']" :title=" record.state==='ON'?'启用':'禁用' " >
            <a-switch size="small" :checked="record.state === 'ON'" @change="toggleState(record)" />
          </a-tooltip>
          <a-button
            v-permission="['sysDict:add']"
            class="rowBtn"
            type="link"
            size="small"
            @click="handleAddChildren(record)">添加下级</a-button>
          <a-button
            v-permission="['sysDict:update']"
            class="rowBtn"
            type="link"
            size="small"
            @click="handleEdit(record)">编辑</a-button>
          <a-popconfirm
            v-permission="['sysDict:remove']"
            placement="top"
            title="你确定要删除这条数据吗?"
            trigger="hover"
...
import { filterObj, modalForm } from '@/utils/util'
export default {
  name: 'DataDict',
  data () {
    return {
