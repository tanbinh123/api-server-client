<template>
  <a-card :bordered="false">
    <single-search-form :searchFields="searchFields" @search="handleSearchEvent">
      <template v-if="checkPermission(['sysResource:add'])" #head>
    ...
    <a-table
      ...
      <template slot="action" slot-scope="record">
        <div class="row-operations">
          <a-tooltip v-permission="['sysResource:toggle']" :title="record.state==='ON'?'启用':'禁用' " >
            <a-switch size="small" :checked="record.state === 'ON'" @change="toggleState(record)" />
          </a-tooltip>
          <a-button
            v-if="record.type === 'menu'"
            v-permission="['sysResource:add']"
            class="rowBtn"
            type="link"
            size="small"
            @click="handleAddChildren(record)">添加下级</a-button>
          <a-button
            v-permission="['sysResource:update']"
            class="rowBtn"
            type="link"
            size="small"
            @click="handleEdit(record)">编辑</a-button>
          <a-popconfirm
            v-permission="['sysResource:remove']"
            placement="top"
            title="你确定要删除这条数据吗?"
            trigger="hover"
